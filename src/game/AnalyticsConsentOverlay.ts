...
export class AnalyticsConsentOverlay {
  ...
  constructor(scene: Phaser.Scene, onChoice: (allowed: boolean) => void) {
    ...
    const title = makeText(scene, cx, portrait ? py(288) : 270, '익명 플레이 기록을 허용할까요?', portrait ? 21 : 27, UI.text, true).setOrigin(0.5);
    const body = makeText(
      scene,
      cx,
      portrait ? py(340) : 326,
    ...
    const privacy = makeText(scene, cx, portrait ? py(470) : 410, '개인정보 안내 보기  →', 12, UI.textDim, true)
      .setOrigin(0.5).setInteractive({ useHandCursor: true });
    privacy.on('pointerdown', () => window.open('./privacy.html', '_blank', 'noopener'));
    const deny = makeButton(scene, portrait ? 104 : 535, portrait ? py(550) : 470, portrait ? 158 : 180, portrait ? 52 : 46, '허용 안 함', () => {
      this.root.destroy(true);
      onChoice(false);
    }, { fill: UI.panelDeep, textColor: UI.textDim, strokeAlpha: 0.18 });
    const allow = makeButton(scene, portrait ? 286 : 745, portrait ? py(550) : 470, portrait ? 158 : 180, portrait ? 52 : 46, '익명 기록 허용', () => {
      this.root.destroy(true);
      onChoice(true);
