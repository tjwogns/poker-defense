import Phaser from 'phaser';
import { UNIT_DEFS } from '../core/units';
import { UI, makeButton, makeText } from './ui';
import { handbookRows } from './guideData';
import { HandRank, isHiddenHand } from '../core/cards/types';
import { isPortraitLayout } from './device';
import { portraitSceneHeight, portraitY } from './layout';
import { tr } from '../i18n';

/** 족보 성립 조건과 획득 유닛을 게임 중 확인하는 모달 도감. */
export class GuideOverlay {
  private root: Phaser.GameObjects.Container;

  constructor(scene: Phaser.Scene, discoveredHands: readonly HandRank[], onClose: () => void) {
    if (isPortraitLayout()) {
      this.root = this.createPortrait(scene, discoveredHands, onClose);
      return;
    }
    const children: Phaser.GameObjects.GameObject[] = [];
    const rows = handbookRows();
    const dim = scene.add.rectangle(640, 360, 1280, 720, 0x020705, 0.86).setInteractive();
    const shadow = scene.add.rectangle(640, 363, 1100, 660, 0x000000, 0.45);
    const panel = scene.add.rectangle(640, 356, 1100, 660, UI.panel, 1)
      .setStrokeStyle(2, UI.panelGlow, 0.95);
    children.push(dim, shadow, panel);

    children.push(
      makeText(scene, 120, 54, 'POKER DEFENSE ARCHIVE', 11, UI.accentText, true),
      makeText(scene, 120, 73, tr('족보 · 유닛 도감', 'HAND & UNIT GUIDE'), 28, UI.text, true),
      makeText(scene, 120, 108, tr('낮은 족보부터 높은 족보 순서입니다. 족보를 확정하면 대응 유닛 1기를 얻습니다.', 'Hands are listed from lowest to highest. Confirm a hand to recruit its unit.'), 13, UI.textDim),
    );

    const close = makeButton(scene, 1110, 78, 110, 36, tr('닫기  ESC', 'CLOSE  ESC'), onClose, {
      fill: 0x42544a,
      fontSize: 11,
    });
    children.push(close.container);

    const headerY = 144;
    children.push(
      makeText(scene, 130, headerY, tr('족보', 'HAND'), 11, UI.gold, true),
      makeText(scene, 330, headerY, tr('성립 조건', 'REQUIREMENT'), 11, UI.gold, true),
      makeText(scene, 635, headerY, tr('획득 유닛', 'UNIT'), 11, UI.gold, true),
      makeText(scene, 795, headerY, tr('전투 특성', 'TRAIT'), 11, UI.gold, true),
    );
    const divider = scene.add.rectangle(640, 166, 1020, 1, UI.panelLine, 1);
    children.push(divider);

    rows.forEach((row, index) => {
      const y = 181 + index * 31;
      const def = UNIT_DEFS[row.rank];
      if (index % 2 === 0) {
        children.push(scene.add.rectangle(640, y, 1010, 28, UI.panelRaised, 0.7));
      }
      const hidden = isHiddenHand(row.rank);
      const discovered = !hidden || discoveredHands.includes(row.rank);
      const badge = scene.add.rectangle(112, y, 5, 20, discovered ? def.color : 0x59645e, 1);
      children.push(
        badge,
        makeText(scene, 130, y, discovered ? row.hand : '???', 12, discovered ? UI.text : UI.gold, true).setOrigin(0, 0.5),
        makeText(scene, 330, y, discovered ? row.rule : tr('덱을 개조해 비밀 족보를 발견하세요', 'Modify your deck to discover this secret hand'), 11, UI.textDim).setOrigin(0, 0.5),
        makeText(
          scene,
          635,
          y,
          discovered ? `${def.glyph}  ${row.unit}` : tr('잠김', 'LOCKED'),
          11,
          discovered ? `#${def.color.toString(16).padStart(6, '0')}` : UI.textDim,
          true,
        ).setOrigin(0, 0.5),
        makeText(scene, 795, y, discovered ? row.trait : tr('한 번 완성하면 영구 공개', 'Complete once to reveal permanently'), 11, UI.textDim).setOrigin(0, 0.5),
      );
    });

    children.push(
      makeText(scene, 120, 582, 'SPECIAL', 9, UI.gold, true),
      makeText(scene, 185, 582, tr('마운틴 10-J-Q-K-A: 피해 +25% · 백스트레이트 A-2-3-4-5: 공격 속도 +18%', 'Broadway 10-J-Q-K-A: damage +25% · Wheel A-2-3-4-5: attack speed +18%'), 9, UI.text, true),
      makeText(scene, 120, 602, 'SUITS', 9, UI.accentText, true),
      makeText(scene, 170, 602, tr('♠ 보스 +12% · ♥ 공속 +11% · ♦ 처치 +1G(라운드 3G) · ♣ 일반 적 +10% · 2-2-1은 직접 선택', '♠ Boss +12% · ♥ Speed +11% · ♦ Kill +1G (3G/round) · ♣ Normal +10% · choose 2-2-1'), 9, UI.text, true),
      makeText(scene, 120, 622, 'BUILD', 9, UI.accentText, true),
      makeText(scene, 170, 622, tr('유물 · 족보 연마 · 대표 문양을 조합해 주력 화력을 완성하세요', 'Combine relics, hand mastery, and lead suits to focus your firepower'), 9, UI.text, true),
      makeText(scene, 120, 642, 'FUSION', 9, UI.gold, true),
      makeText(scene, 170, 642, tr('동일 3기 선택 합성 · 첫 기준 유닛의 위치와 대표 문양 계승', 'Fuse 3 matching units · inherits the first unit\'s position and lead suit'), 9, UI.text, true),
      makeText(scene, 120, 668, tr('H로 언제든 도감을 열 수 있습니다 · 히든 족보는 첫 발견 후 조건과 유닛이 공개됩니다.', 'Press H anytime to open the guide · secret hands are revealed after discovery.'), 9, UI.textDim),
    );

    this.root = scene.add.container(0, 0, children).setDepth(40);
  }

  private createPortrait(scene: Phaser.Scene, discoveredHands: readonly HandRank[], onClose: () => void): Phaser.GameObjects.Container {
    const height = portraitSceneHeight(scene);
    const py = (value: number) => portraitY(height, value);
    const children: Phaser.GameObjects.GameObject[] = [];
    const rows = handbookRows();
    children.push(
      scene.add.rectangle(195, height / 2, 390, height, 0x020705, 0.9).setInteractive(),
      scene.add.rectangle(195, height / 2, 370, height - 24, UI.panel, 1).setStrokeStyle(1, UI.panelGlow, 0.95),
      makeText(scene, 20, py(28), 'POKER DEFENSE ARCHIVE', 9, UI.accentText, true),
      makeText(scene, 20, py(48), tr('족보 · 유닛 도감', 'HAND & UNIT GUIDE'), 22, UI.text, true),
    );
    children.push(makeButton(scene, 338, py(48), 76, 38, tr('닫기', 'CLOSE'), onClose, { fill: 0x42544a, fontSize: 11 }).container);
    const top = py(100);
    const bottom = height - 46;
    const gap = (bottom - top) / rows.length;
    rows.forEach((row, index) => {
      const y = top + gap * index;
      const def = UNIT_DEFS[row.rank];
      const hidden = isHiddenHand(row.rank);
      const discovered = !hidden || discoveredHands.includes(row.rank);
      if (index % 2 === 0) children.push(scene.add.rectangle(195, y + gap / 2, 350, gap - 2, UI.panelRaised, 0.7));
      children.push(
        scene.add.rectangle(24, y + 5, 4, Math.max(18, gap - 8), discovered ? def.color : 0x59645e, 1).setOrigin(0, 0),
        makeText(scene, 36, y + 3, discovered ? row.hand : '???', 11, discovered ? UI.text : UI.gold, true),
        makeText(scene, 150, y + 3, discovered ? `${def.glyph} ${row.unit}` : tr('잠김', 'LOCKED'), 10, discovered ? `#${def.color.toString(16).padStart(6, '0')}` : UI.textDim, true),
        makeText(scene, 36, y + 19, discovered ? `${row.rule} · ${row.trait}` : tr('덱을 개조해 비밀 족보를 발견하세요', 'Modify your deck to discover this secret hand'), 8, UI.textDim)
          .setWordWrapWidth(325, true),
      );
    });
    children.push(makeText(scene, 195, height - 27, tr('히든 족보는 첫 발견 후 조건과 유닛이 공개됩니다.', 'Secret hands are revealed after you discover them.'), 9, UI.textDim).setOrigin(0.5));
    return scene.add.container(0, 0, children).setDepth(40);
  }

  destroy(): void {
    this.root.destroy(true);
  }
}
